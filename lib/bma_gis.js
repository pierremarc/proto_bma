...
window.BG = {
    ...
    install_map: function(container, observers){
        ...
        this.map.removeControl(this.map.zoomControl).addControl(new this.ctrl_zoom({position:'bottomleft'}));
    ...
    install_features:function(map_id){
        var map = this.map;
        if(map_id !== undefined)
        ...
        this.get_all(function(data){
            ...
            for(var idx=0; idx < data.length; idx++)
            {
                ...
                var res = new L.MultiPolygon(multi_polygon);
                this.features_.push({pid:d.pid, feature:res});
